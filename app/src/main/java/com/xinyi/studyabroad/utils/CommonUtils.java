...
public class CommonUtils {
    ...
    public static Locale getUserLocale(Context pContext) {
        String _LocaleJson = (String) SpUtils.get(pContext, LOCALE_KEY, "");
    ...
    public static void saveUserLocale(Context pContext, Locale pUserLocale) {
        ...
        SpUtils.put(pContext, LOCALE_KEY, _LocaleJson);
    }
     ...
     *    
     */
    public static boolean needUpdateUserLocale(Context pContext, Locale pNewUserLocale) {
        return pNewUserLocale != null && (getUserLocale(pContext) == null || !getUserLocale(pContext).equals(pNewUserLocale));
    }

    public static void setLanguage(Locale ll, Context context) {

        if (needUpdateUserLocale(context, ll)) {
            Configuration _Configuration = getResources().getConfiguration();
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                _Configuration.setLocale(ll);
            } else {
                _Configuration.locale = ll;
            }
